...
def write_preserving_excel_features(source: Path, output: Path,
    ...
    """Patch only worksheet cell XML; preserve every other XLSM part verbatim."""
...
def extract_values(path: Path, country: str) -> dict[str, Any]:
    ...
    if country == "India":
        # 印度供应商目前存在两种文件：印度单区域模板，以及沿用中国双区域模板
        # 的文件。国家仍由汇总表第 12 行决定，模板形态再由关键名称决定。
        try:
            find_rightmost_label(ws, "+ ICC ON RM", exact=True)
        except LookupError:
            return extract_china_values(ws, ds)
...
def main() -> int:
    # PyInstaller 单文件 EXE 会把源码释放到临时目录；此时必须使用 EXE 所在目录。
    ...
    for sheet_name in requested_sheets:
        ...
        for col in SUPPLIER_COLUMNS:
            ...
            if country is None:
                item.update(
                    status="skipped",
                    error=f"第 12 行国家不是 China 或 India: {country_value!r}",
                )
